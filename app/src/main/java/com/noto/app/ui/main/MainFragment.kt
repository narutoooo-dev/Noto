...
import com.noto.app.ui.filtered.filteredItem
...
//        rv.edgeEffectFactory = BounceEdgeEffectFactory()
    ...
    private fun MainFragmentBinding.setupAllFolders(
        ...
        notesCount: FilteredItem.NotesCount,
        isVaultOpen: Boolean,
    ) {
        context?.let { context ->
            rv.withModels {
                    ...
                    onSuccess = { folders ->
                        ...
                        group(R.layout.vertical_linear_layout_group) {
                            ...
                            group(R.layout.horizontal_linear_layout_group) {
                                ...
                                filteredItem {
                                    id("all")
                                    model(FilteredItem.All)
                                    notesCount(notesCount.all)
                                    isShowNotesCount(isShowNotesCount)
                                    isSelected(FilteredItem.All.id == selectedDestinationId)
                                    onClickListener { _ ->
                                        dismiss()
                                        if (selectedDestinationId != FilteredItem.All.id)
                                ...
                                filteredItem {
                                    id("recent")
                                    model(FilteredItem.Recent)
                                    notesCount(notesCount.recent)
                                    isShowNotesCount(isShowNotesCount)
                                    isSelected(FilteredItem.Recent.id == selectedDestinationId)
                                    onClickListener { _ ->
                                        dismiss()
                                        if (selectedDestinationId != FilteredItem.Recent.id)
                                            navController?.navigateSafely(
                                                MainFragmentDirections.actionMainFragmentToFilteredFragment(
                                                    FilteredItem.Recent
                                                )
                            ...
                            group(R.layout.horizontal_linear_layout_group) {
                                ...
                                filteredItem {
                                    id("scheduled")
                                    model(FilteredItem.Scheduled)
                                    notesCount(notesCount.scheduled)
                                    isShowNotesCount(isShowNotesCount)
                                    isSelected(FilteredItem.Scheduled.id == selectedDestinationId)
                                    onClickListener { _ ->
                                        dismiss()
                                        if (selectedDestinationId != FilteredItem.Scheduled.id)
                                            navController?.navigateSafely(
                                                MainFragmentDirections.actionMainFragmentToFilteredFragment(
                                ...
                                filteredItem {
                                    id("archived")
                                    model(FilteredItem.Archived)
                                    notesCount(notesCount.archived)
                                    isShowNotesCount(isShowNotesCount)
                                    isSelected(FilteredItem.Archived.id == selectedDestinationId)
                                    onClickListener { _ ->
                                        dismiss()
                                        if (selectedDestinationId != FilteredItem.Archived.id)
                                            navController?.navigateSafely(
                                                MainFragmentDirections.actionMainFragmentToFilteredFragment(
