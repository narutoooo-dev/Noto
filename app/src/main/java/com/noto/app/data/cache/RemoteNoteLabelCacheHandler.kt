...
class RemoteNoteLabelCacheHandler(
    ...
    override suspend fun cacheRemoteItems(remoteItems: List<RemoteNoteLabel>) {
        ...
        val databaseLocalNoteLabels = localNoteLabelDataSource.getNoteLabelsByNoteId(localNoteId).first()
        ...
        newNoteLabels.forEach { localNoteLabelDataSource.createNoteLabel(it) }
        deletedNoteLabels.forEach { localNoteLabelDataSource.deleteNoteLabel(it) }
    }
}